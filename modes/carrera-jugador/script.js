const STORAGE_KEY = "torre_futbol_carrera_jugador_v1";
...
function renderMarket() {
  $("#offersList").innerHTML = state.offers.length
    ? state.offers.map((offer, index) => `<div class="offer-card">
        <header><h3>${offer.club}</h3><strong>${moneyText(offer.salary)}/sem</strong></header>
...
function playMatch() {
  ...
  $("#matchResult").innerHTML = `<h3>${state.club} ${teamGoals} - ${rivalGoals} ${state.nextOpponent}</h3>
    <p>Tu partido: media ${rating}, ${goals} goles, ${assists} asistencias${cleanSheet ? ", valla invicta" : ""}.</p>`;
  addNews(`${state.club} ${teamGoals}-${rivalGoals} ${state.nextOpponent}. Media personal ${rating}.`);
...
function advanceWeek() {
  ...
  state.nextOpponent = randomOpponent();
  if (state.injuryWeeks > 0) state.injuryWeeks -= 1;
